<script>
  ...
  function setupChart() {
    ...
    const container = d3.select('#custom-chart-container')
      .select("svg")
      .remove(); // Remove any existing chart

    const svg = d3.select('#custom-chart-container')
      .append('svg')
...
<div id="custom-chart-container"></div>
